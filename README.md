...
出牌记录的浏览器回归测试使用固定公开牌例驱动真实牌桌组件，覆盖完整历史、当前墩、查看旧记录时的位置保持、跟随最新、下一副重置及电脑和手机布局。首次运行需安装测试浏览器；此依赖仅用于开发测试，不影响游戏离线运行。
...
最后一名真人的连接断开被服务端检测到后，牌桌立即暂停并保存，电脑不能继续行动或自动开始下一副。重连仅取回座位控制，仍需当前房主点击“恢复牌桌”。暂停保存失败时，服务端阻止电脑继续推进并重试保存。

房主离线满 30 秒后，权限转交给加入最早且仍在线的真人；无人在线时保留原房主，待符合条件的真人重连后再转交。权限转交不会解除暂停，原房主回归也不会自动收回权限。

服务启动时恢复已提交的叫牌、出牌、结算、准备、累计分及操作去重记录，已有牌局保持暂停。已有离线截止时间不会重置，此前在线的身份从启动检测时开始等待 30 秒。启动恢复无法保存时服务拒绝启动，修复存储后可重试。恢复快照继续按身份过滤，只提供本人手牌与当前阶段允许公开的信息。
