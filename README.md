...
面向熟人的单机与局域网桥牌游戏。当前已实现昵称建房、房间码入房、四方选座、房主开局、随机发牌、私有手牌展示、叫牌与定约认定、十三墩打牌、结算与下一副准备、实时同步和刷新恢复。电脑按已确认约定叫牌，并完成基础首攻、做庄、明手控制与防守。
...
房主先入座，再点击“开始第一副”。服务端检查房主权限、预期版本和牌局阶段，随机分配完整 52 张牌，每家 13 张。支持 1～4 个真人座位，空位标记为电脑座位。首副为第 1 副、北家发牌、双方无局、北家首先叫牌。电脑牌手自动叫牌和出牌，轮到真人或真人庄家控制明手时等待操作。
...
`board.auction` 保存按次序排列的完整公开记录；`board.legalCalls` 仅对当前行动的真人身份提供。叫牌与版本、操作记录同事务提交，失败或过期不改变已确认状态。旧版已发牌存档按空叫牌记录继续使用。出牌、结算及准备下一副使用相同授权与事务路径；电脑决策在独立 Worker 中执行，过期结果不能推进牌局。
